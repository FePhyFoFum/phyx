...
bool get_nexus_translation_table(std::istream& stri, std::map<std::string, std::string> * trans, std::string * retstring);
bool check_nexus_comment(std::string line);
void process_nexus_comment(std::istream& stri, std::string& tline);
Tree * read_next_tree_from_stream_nexus(std::istream& stri, std::string& retstring, bool ttexists,
    std::map<std::string, std::string> * trans, bool * going);
